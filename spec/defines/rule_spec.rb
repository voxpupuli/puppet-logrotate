...
describe 'logrotate::rule' do
  on_supported_os.each do |os, os_facts|
    context "on #{os}" do
      ...
      when 'FreeBSD'
        config_path = '/usr/local/etc/logrotate.d/'
        group = 'wheel'
      else
      ...
      context 'ensure => absent' do
        ...
        it { is_expected.to contain_file("#{config_path}test").with_ensure('absent') }
      end
      ...
      context 'valid title' do
        ...
        include_context 'config file', path: '/var/log/foo.log' do
          let(:config_file) { "#{config_path}test" }
          let(:space_prefix) { '  ' }
        end
        ...
        describe 'base defaults' do
          ...
          it do
            ...
            is_expected.to contain_file("#{config_path}test").with(
              'owner' => 'root',
              'group' => group,
              'ensure' => 'present',
              'mode' => '0644'
        ...
        context 'with an array path' do
          ...
          it {
            is_expected.to contain_file("#{config_path}test").
        ...
        context 'and create => true' do
          ...
          describe 'and create_mode => 0777' do
            ...
            it {
              is_expected.to contain_file("#{config_path}test").with_content(%r{^  create 0777$})
            }
          end
        ...
        context 'and create => false' do
          ...
          it do
            is_expected.to contain_file("#{config_path}test").with_content(%r{^  nocreate$})
          end
        ...
        context 'and rotate => 3' do
          ...
          it { is_expected.to contain_file("#{config_path}test").with_content(%r{^  rotate 3$}) }
        end
        ...
        context 'and rotate_every => hour' do
          ...
          it { is_expected.to contain_file("#{config_path}hourly/test") }
          it { is_expected.to contain_file("#{config_path}test").with_ensure('absent') }
        end
        ...
        context 'and rotate_every => day' do
          ...
          it { is_expected.to contain_file("#{config_path}test").with_content(%r{^  daily$}) }

          it do
            is_expected.to contain_file("#{config_path}hourly/test").
              with_ensure('absent')
          end
        end
      ...
      context 'with a non-alphanumeric title' do
        ...
        it {
          expect do
            is_expected.to contain_file("#{config_path}foo bar")
    ...
    /var/log/wtmp {
      ...
      context 'template should not inherit variables from other scopes', if: os_facts[:os]['family'] == 'RedHat' && os_facts[:os]['release']['major'].to_i == 7 do
        let(:title) { 'foo' }
        let(:params) do
          {
            path: '/var/log/foo.log',
        ...
        it { is_expected.to contain_file("#{config_path}btmp").without_content(%r{/ifempty/}) }
      end
    end
  end
end
